...
<template>
    <div class="pt-4 pb-4 px-3">
        ...
        <div class="card">
            <div class="card-header">
                ...
                <!-- <div class="card-tools" style="display: inline;">
                  ...
                  <Link href="/add-students" class="ml-4 btn btn-primary">Add parents</Link>
                </div> -->
                <div class="card-tools">
                    <div class="input-group input-group-sm" style="width: 450px;">
                        ...
                        <Link href="/add-parents-only" class="ml-4 btn btn-primary">Add parents</Link>
                    </div>
            ...
            <div class="card-body table-responsive p-0">
                <table class="table table-hover text-nowrap">
                    ...
                    <tbody v-if="allParents.data.length > 0">
                        <tr v-for="(parent, index) in allParents.data" :key="index">
                            <td>{{ index + 1 }}</td>
                            <td><a href="#" @click="viewParent(index)">{{ parent.fullname }}</a></td>
                            <td>{{ parent.email }}</td>
                            <td>{{ parent.phone }}</td>
                            <td>
                                <a href="#" v-if="parent.students.length > 0"
                                    @click="reAssign(index, parent.id)">Reassign</a>
                                <a href="#" @click="assign(index, parent.id)" v-else>Assign</a>
                            </td>
                            <td ref="totalstudent">{{ parent.students.length }}</td>
                            <td>
                                <Link href="/edit-parent-only" :data="{ id: parent.id }"><i
                                    class="fas fa-edit text-success"></i></Link>
                                <a href="#" @click.prevent="deleteParent(parent.id, parent.fullname)"><i
        ...
        <!-- View parent Modal -->
        <div class="modal fade" id="viewparent" tabindex="-1" role="dialog" aria-labelledby="viewParentModalLabel"
            aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="viewParentModalLabel"><strong>{{ parent.fullname }}</strong>,
                            {{ parent.isparent }} to {{ parent.students ? parent.students.length : 'unassigned' }} Children</h5>
                                ...
                                <ul class="list-group list-group-unbordered mb-3">
                                    ...
                                    <li class="list-group-item">
                                        <b>Email</b> <a class="float-right"
                                            :href="'mailto:' + parent.email">{{ parent.email }}</a>
                                    </li>
                                    <li class="list-group-item">
                                        <b>Phone</b> <a class="float-right"
                                            :href="'tel:' + parent.phone">{{ parent.phone }}</a>
                                    </li>
                                    <li class="list-group-item">
        ...
        <!--Assign Modal -->
        <div class="modal fade" id="assign" tabindex="-1" role="dialog" aria-labelledby="assignModalLabel"
            aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <div class="modal-body">
                        <div id="accept-child">
                            <transition name="fade">
                                <ul class="tags">
                                    <li class="tag-item" v-for="(tag, index) in tags" :key="index">{{ tag.fullname }} <span
                                            @click="close(index, tag)"><i class="fas fa-times"></i></span></li>
                                </ul>
                            </transition>
                            <input type="text" autofocus ref="search_input" placeholder="search by name or student ID"
                                v-model="search" class="form-control no-border-input">
                        </div>
                        <transition name="fade">
                            <ul class="no-style" v-if="students.length > 0">
                                <li v-for="(std, index) in students" :key="index"
                                    :class="{ 'text-danger': std.parents_count >= 2 }" @click="selectStudent(index, std)">
                                    {{ std.fullname + " (" + std.grade + ")" }}
                                    <span v-if="std.parents_count >= 2" class="text-danger pl-2">[Cannot assign this
                                        student]</span>
                                </li>
                            </ul>
                        ...
                        <div class="text-center mt-2">
                            <form @submit.prevent="assignStudent(tags)">
                                <input v-if="tags.length > 0" :disabled="isprocessing" type="submit"
...
export default {
    ...
    methods: {
        ...
        searchParents(query) {
            axios.get('/search-parents', { params: { search: query } })
                .then((response) => {
                    this.allParents.data = response.data.data
