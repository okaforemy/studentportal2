    ...
    <div class="col-md-12 py-4">
        ...
        <div class="pt-4 ">
            <table class="table table-striped col-md-12 table-md-responsive result-table">
                ...
                <tbody>
                    <tr v-for="(subject, index) in student.senior_secondary_exam" :key="index">
                        <td>{{ index +1 }}</td>
                        <td class=" text-uppercase parent-td">{{ subject.subject }} <span @click="deleteSubject(subject.id)" class="text-danger cursor-pointer"><i class="fas fa-trash"></i></span></td>
            ...
            <h4 class="text-center">Average: <span class="pl-2" v-if="student && student.class_avg">{{ (student.class_avg).toFixed(2) }}</span></h4>
        </div>
        ...
        methods:{
            ...
            deleteSubject(id){
                if (confirm("Do you really want to delete this item?")) {
                    axios.get('/delete-senior-secondary-exam-subject', {params: {id: id}})
                .then((response)=>{
                    axios.get('/get-midterm-result', {params:{student_id: this.student_id, section: this.section}})
                    .then((response)=>{
                        this.student = response.data
            ...
            //     let remark = '';
            //     let total = ((val/15)*100).toFixed(1);
            //     if(total >= 80){
            //         remark = "A1"
            ...
            // },
             grading(val) {
                let score = ((val/15) * 100).toFixed(2);
                if (score >= 75) return "A1";
                if (score >= 70 && score < 75) return "B2";
                if (score >= 65 && score < 70) return "B3";
                if (score >= 60 && score < 65) return "C4";
                if (score >= 55 && score < 60) return "C5";
                if (score >= 50 && score < 55) return "C6";
                if (score >= 45 && score < 50) return "D7";
                if (score >= 40 && score < 45) return "E8";
