                                ...
                                <table class="table">
                                    ...
                                    <tbody v-if="subjects.length > 0">
                                        <tr v-for="(subj, index) in subjects" :key="index">
                                            ...
                                            <td>
                                                <input type="text" :value="subj.first_ca" @keypress="isNumber" @blur="total($event,20,index,subj.first_ca)" @keyup="total($event,20,index,subj.first_ca)" name="first_ca" :id="'first_ca_'+index" class="form-control" placeholder="first CA">
                                            </td>
                                            <td>
                                                <input type="text" :value="subj.second_ca" @keypress="isNumber" @blur="total($event,20,index,subj.second_ca)" @keyup="total($event,20,index,subj.second_ca)" name="second_ca" :id="'second_ca_'+index" class="form-control" placeholder="second CA">
                                            </td>
                                            <td>
                                                <input type="text" :value="subj.exam" @keypress="isNumber" @blur="total($event,60,index,subj.exam)" @keyup="total($event,60,index,subj.exam)" name="exam" :id="'exam_'+index" class="form-control" placeholder="exam">
                                            </td>
                                            <td>
...
export default {
    ...
    methods:{
        ...
        total(evt,max,index,oldval){
            let val = evt.target.value;
            let old_val = oldval? oldval:''
            if(val > max){
                toastr.error('Maximum number entered!', 'Error')
                evt.target.value=old_val;
               // return true;
            }
